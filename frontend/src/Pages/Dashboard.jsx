...
const Dashboard = () => {
  const { user } = useAuth();
  const { courses, isLoading } = useCourses(); // Ensure isLoading is exposed in Context
  const [searchQuery, setSearchQuery] = useState("");
  const [isModalOpen, setIsModalOpen] = useState(false);
  const navigate = useNavigate();

  // Get the most recent course safely
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-5xl mx-auto px-6 py-8 md:px-12 md:py-12">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
            ...
            {/* Stats Pills - Optional: You can make these dynamic later */}
          ...
          {isLoading ? (
            ...
            <>
              {/* Featured / Jump Back In Section - ONLY RENDER IF COURSE EXISTS */}
              {featuredCourse && (
                <section className="mb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
                  ...
                  <div className="group relative overflow-hidden rounded-3xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-xl shadow-zinc-200/50 dark:shadow-black/50 hover:border-zinc-300 dark:hover:border-zinc-700 transition-all duration-300">
                    <div className="flex flex-col md:flex-row">
                      {/* Thumbnail Side */}
                      <div className="relative md:w-2/5 h-48 md:h-auto overflow-hidden bg-zinc-100 dark:bg-zinc-800">
                      ...
                      <div className="flex-1 p-6 md:p-8 flex flex-col justify-center">
                        ...
                        <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6">
                          {/* Fallback if lastWatchedVideoId isn't populated with a title yet */}
              ...
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  {courses.map((course) => (
                    <Link
                      to={`/course/${course._id}`} // Use _id here
                      key={course._id} // Use _id here
                      className="group relative bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl overflow-hidden hover:shadow-lg hover:shadow-zinc-200/50 dark:hover:shadow-black/50 transition-all duration-300 hover:-translate-y-1 block"
                    >
