...
const CourseViewer = () => {
  ...
  // Early exit if course not found or no videos
  if (!course || course.videos.length === 0) {
    return (
      <div className="p-10 text-center dark:text-white">
        Course not found or has no videos loaded.
      </div>
    );
  }

  // Find currently active video object and its index
  ...
  const handleMarkComplete = () => {
    ...
    // Optional: Auto-advance to next video after marking complete
    // handleNext();
  };
    ...
    <div className="flex h-screen overflow-hidden bg-zinc-50 dark:bg-zinc-950 font-inter">
      ...
      <aside
        className={`
            ...
            flex flex-col h-full border-r border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden relative
        `}
      >
        {/* Sidebar Header */}
        ...
        <div className="flex-1 overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-zinc-200 dark:scrollbar-thumb-zinc-800">
          {course.videos.map((video, index) => {
            ...
            // A video is "locked" if the previous one isn't complete (optional logic, disabled for now for flexibility)
            // const isLocked = index > 0 && !course.videos[index - 1].completed;
            const isLocked = false; // Allowing free navigation for now
              ...
              >
                <div className="shrink-0 mt-0.5">
                  {video.completed ? (
                    ...
                    // Empty circle placeholder for unwatch state
      ...
      <main className="flex-1 flex flex-col h-full overflow-y-auto relative relative">
        {/* Expand Sidebar Button (Visible only when sidebar closed) */}
        {!isSidebarOpen && (
          <button
            onClick={() => setIsSidebarOpen(true)}
            className="absolute top-4 left-4 z-10 p-2 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-sm text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100 transition-colors"
          >
            <SidebarOpen size={20} />
          </button>
        )}

        {/* Top Navigation / Breadcrumbs (Simplified for now) */}
        <header className="h-16 shrink-0 border-b border-zinc-200 dark:border-zinc-800 bg-white/50 dark:bg-zinc-950/50 backdrop-blur-xl px-6 md:px-8 flex items-center justify-between sticky top-0 z-10">
          <div className="flex items-center gap-2 text-sm">
            <span className="text-zinc-500">My Courses</span>
            <ChevronRight size={14} className="text-zinc-400" />
            <span className="font-medium text-zinc-900 dark:text-zinc-100 truncate max-w-[200px]">
              {course.title}
            </span>
          </div>
          {/* Add User Profile / Dark Mode toggle here if desired */}
        </header>
        ...
        <div className="flex-1 p-6 md:p-8 max-w-5xl mx-auto w-full">
          ...
          <div className="aspect-video bg-black rounded-2xl overflow-hidden shadow-xl relative z-0">
            {/* Using iframe for simplicity. Ensure YouTube API is allowed. */}
            <iframe
              width="100%"
              height="100%"
          ...
          <div className="text-sm text-zinc-600 dark:text-zinc-300 leading-relaxed">
            {activeTab === "description" && (
              <p>
                ...
                (Placeholder description based on video title).
              </p>
            )}
            {activeTab === "notes" && (
